...
- **Scheduled or manual dispatcher run:** the workflow automatically refreshes known draft API diff PRs, leaves non-drafts alone, ensures a major-to-major comparison is represented (using `ga` for the previous release and `*` to find the latest current version), and kicks off one no-input worker run to discover the next milestone.
