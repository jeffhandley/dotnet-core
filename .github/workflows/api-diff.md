...
Use `release-notes/RunApiDiff-CollectAssemblies.ps1` and `release-notes/RunApiDiff-GenerateReport.ps1` to generate exactly one API diff comparison. Create or refresh the matching pull request using the configured safe outputs. You may consult `@.github/skills/api-diff` as a reference for parameter mapping.

## Operating rules

1. Keep the agent job read-only. Use the configured safe outputs for all PR creation, PR updates, and branch refreshes.
2. The pull request must contain **only** the files generated by the `RunApiDiff-GenerateReport.ps1` script. No other file changes — regardless of location — may be included.
3. If there are no file changes after generation, do not create a PR.
4. **NEVER modify, create, edit, or delete any file that was not produced by the RunApiDiff scripts.** Any changes outside the scripts' output are strictly prohibited.
...
7. DO NOT edit or alter any of the files produced by the scripts while preparing or creating the pull request.
...
14. **Preflight check:** Before running any generation scripts, verify that `push_to_pull_request_branch`, `update_pull_request`, and `noop` safe output tools are available. If they are not, write a summary explaining that safe output tools are unavailable, and stop immediately. Do not attempt to call MCP tools via HTTP, curl, or any other workaround — only use the tools available in your tool surface.
...
Save the JSON manifest from Step 2 to a temporary file and pipe it into `RunApiDiff-GenerateReport.ps1`:

```bash
# Save manifest to temp file (the agent captured stdout from Step 2)
cat > /tmp/api-diff-manifest.json << 'MANIFEST_EOF'
{MANIFEST_JSON_FROM_STEP_2}
MANIFEST_EOF

# Generate reports — installs apidiff tool, runs all SDKs, creates README, normalizes newlines
...
The manifest contains these fields needed for Step 4:
...
### Step 4 — Create or refresh the pull request
...
3. Stage **only** the files generated by the scripts and commit them. Do not use `git add .` or `git add -A` — only add the specific api-diff output directory. No other files may be staged or committed:
   ```bash
