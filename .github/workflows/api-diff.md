...
steps:
  - name: Generate API diff
    env:
      INPUT_PREVIOUS_MAJOR_MINOR: ${{ inputs.previous_major_minor }}
      INPUT_PREVIOUS_LABEL: ${{ inputs.previous_label }}
      INPUT_CURRENT_MAJOR_MINOR: ${{ inputs.current_major_minor }}
      INPUT_CURRENT_LABEL: ${{ inputs.current_label }}
    run: bash release-notes/generate-api-diff.sh
...
The API diff files have already been generated by the deterministic `generate-api-diff.sh` step that ran before this agent started. The generated files are in the workspace and metadata is at `/tmp/gh-aw/agent/metadata.json`. Your job is to inspect the results and create or update the appropriate pull request using safe outputs.
...
1. Read `/tmp/gh-aw/agent/metadata.json`. It contains:
   ...
   - `reason`: explanation when status is not `"success"`
   - `previous` / `current`: human-friendly version descriptions
   - `is_release_to_release`: whether this is a GA-to-GA cross-major comparison
   - `feed_used`: which NuGet feed was used
   - `branch_name`: suggested branch name (e.g. `api-diff/net11-preview2_net11-preview3`)
   - `pr_title`: suggested PR title suffix (e.g. `.NET 11.0 Preview 2 -> Preview 3`)
   - `explicit_inputs`: whether this was an explicit or inferred run

2. If `status` is `"failed"` or `"skipped"`, invoke `noop` with the `reason` from metadata, write the step summary, and stop.
...
4. Create a new **draft** PR using `create_pull_request` with:
   ...
   - Body: brief summary of the comparison, feed used, and SDKs compared
...
- Whether generation used `dotnet-public` or a fallback feed
