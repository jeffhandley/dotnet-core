...
4. **NEVER modify, create, edit, or delete any file under `.github/` at any point during the run.** This includes workflow `.md` source files, compiled `.lock.yml` files, skill files, and any other configuration. These files are strictly read-only for this agent.
...
3. Stage **only** the generated api-diff files and commit them. Do not use `git add .` or `git add -A` — only add the specific api-diff output directory. Never modify, stage, or include `.github/` files or any other files outside `release-notes/` in the patch:
   ```bash
