#!/usr/bin/env bash
# api-diff-producer-setup.sh
#
# Host-side setup for the API Diff Producer. Given a single discovery target it
...
# agent only stages and narrates. Reviewer feedback is collected by the agent itself
# under gh-aw integrity filtering, not here.
#
...
#
# Inputs:
#   TARGET      required  single discovery target (JSON) from api-diff-manager-discover.cs
...
if [ "$pr_list_failed" = true ]; then
	...
	tracking_block() {
		...
		# VERY LAST thing in the body, delimited by the visible `# api-diff-producer:state:begin`/`:end`
		...
		awk -v b="# api-diff-producer:state:begin" -v e="# api-diff-producer:state:end" '
...
# The agent job checks out with persist-credentials=false and a single-branch
# shallow refspec, so a plain `git fetch origin <branch>` neither authenticates
# (private repo) nor populates refs/remotes/origin/<branch>. Fetch the target
# branch explicitly with a tokened URL and seed from FETCH_HEAD so metadata-only
# refreshes and pending permanent-exclusion edits are honored.
if [ "$noop" = false ] && [ -n "${GH_TOKEN:-}" ] && [ -n "$REPO" ] &&
	git fetch --no-tags --depth=1 "https://x-access-token:${GH_TOKEN}@github.com/${REPO}.git" "$target_branch" >/dev/null 2>&1; then
	echo "::notice::seeding working tree from ${target_branch}"
	git checkout -q FETCH_HEAD -- "$content_dir" 2>/dev/null || true
	git checkout -q FETCH_HEAD -- "$perm_attrs" "$perm_asms" 2>/dev/null || true
fi
mkdir -p "$content_dir"
...
# Every producer run -- publish, metadata-only refresh, blocked, or a silent
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
	...
	{
		echo "## API Diff Producer — ${pr_title}"
		echo
		echo "| Field | Value |"
		echo "| --- | --- |"
		echo "| Track | \`${track}\` |"
