...
// Deterministic discovery for the API Diff Manager, driven entirely by the dnceng
...
//
//   - incremental (milestone deltas): previous public milestone -> latest public
//     milestone. Both ends are NAMED public milestones (preview.N / rc.N) that have
//     shipped to dotnet-public -- never alpha/beta and never a pre-release daily, so
//     the milestone folders stay clean. Content lives under
//     release-notes/X.0/preview/<milestone>/api-diff.
//
...
// Latest NAMED public milestone (rc preferred over preview) shipped to dotnet-public.
async Task<(string Kind, int Num, NuGetVersion Ver)?> LatestPublicMilestone(int major)
{
    if (await MaxMilestone(major, "rc", PublicChannel) is { } rc)
        return ("rc", rc.Num, rc.Ver);
    if (await MaxMilestone(major, "preview", PublicChannel) is { } pv)
...
// The incremental baseline (previous milestone) for <major> at head milestone
// (kind.num), as (version-milestone, version, feed) on dotnet-public -- or null when it has not
// shipped publicly yet (hold-off).
async Task<(string VersionMilestone, string Version, string Feed)?> PrevMilestone(int major, string kind, int num)
{
    ...
    if (kind == "rc")
    {
        // rc.1: baseline is the highest public preview of this major.
...
foreach (var major in new[] { maxGa + 1, maxGa + 2 })
{
    ...
    // incremental: previous public milestone -> latest public milestone.
    var ms = await LatestPublicMilestone(major);
    if (ms is not null)
    {
        ...
        if (pm is not null)
        {
            ...
            targets.Add(new Target(
                ...
                current_version: msVer.ToNormalizedString(), current_feed: FeedIndex(PublicChannel),
...
// Field names/order mirror the JSON contract the API Diff Manager consumes.
record Target(
