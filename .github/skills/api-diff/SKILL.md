...
> **Important:** The pull request must contain **only** the files generated by the `generate_api_diff` MCP tool. Never modify or include any other file — regardless of location — in the patch. Any file not produced by the tool is strictly off-limits.
